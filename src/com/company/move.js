...
function run(carId) {
    ...
    else if (carExamplar.className == "circle startPositionSecond" && parseInt(direction.left) >= 687 && massOfDirections[carId] == 1) {
        if (lengthAnotherCarsBottom()) {
            carExamplar.style.top = parseInt(direction.top) + 15 + 'px';
        }
    } else {
...
function lengthAnotherCarsBottom() {
    var bb = true;
    for (var i = 0; i < massOfThirdBandCars.length; i++) {
        var direction = window.getComputedStyle(massOfThirdBandCars[i], null);
        if (parseInt(direction.left) > 460 && parseInt(direction.left) < 760) {
            bb = false;
        }
    }
    return bb;
